...
def createBranch(
    ...
    if enableGBM or root >= max_depth:  # max depth
        final_decision = subdataset.reset_index()["Decision"][
            subdataset["Decision"].argmax()
        ]
        ...
        subdataset.shape[1] == 1
    ):  # if decision cannot be made even though all columns dropped
        final_decision = (
            subdataset["Decision"].value_counts().idxmax()
    ...
    if enableParallelism != True:
        ...
        functions.storeRule(
            text_file,
            (
                functions.formatRule(root - 1, resp="|", add="   |"),
    ...
    if terminateBuilding:  # check decision is made
        ...
        if enableParallelism != True:
            ...
            functions.storeRule(
                text_file,
                (functions.formatRule(root, resp="|", add="   |"), decision_rule_text),
            )
        else:
            # parallel
            sample_rule = {}
        ...
        for i in range(0, df.shape[0]):
            ...
            if parent_id == parent:
                ...
                if not else_statement:
                    if if_statement and leaf_idx > 0:
                        rule = "el" + rule
